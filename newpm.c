...
int main (int argc, char *argv[])
{
  ...
  homedir = getenv ("HOME");
  if (homedir == NULL) {
    fprintf (stderr, "newpm: Environmental variable '$HOME' must be set in order to run PUD.\n");
    exit (EXIT_FAILURE);
  }
  ...
  if (get_pud_style_data (&output.style, "pm", global) == NULL) {
    exit (EXIT_FAILURE);
  }
  ...
  if (parse_template_file (output.template, after_template, &output) == NULL) {
    fprintf (stderr, "newpm: Failed to parse template file '%s'!\n",
  ...
  if (parse_style_file (output.style, real_output, &output, after_template, &lang) == NULL) {
    fprintf (stderr, "newpm: Failed to parse style file '%s'!\n",
  ...
  if (outfile == NULL) {
    fprintf (stderr, "newpm: Failed to open file '%s' for writing: %s\n",
...
void doHelp (void) {
  /* FIXME */
  printf ("Usage: newpm [FILE] [OPTIONS]\n");
  printf (" OPTIONS LIST\n");
